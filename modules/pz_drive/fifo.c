...
static void fifo_enable_digital(uint8_t gain) {
    drv2665_write_reg(DRV2665_REG_CTRL2, 0x00); // exit standby
...
void pzd_fifo_start(const uint8_t *buf, size_t len, int gain, bool fullwave) {
    ...
    s_gain = (uint8_t)gain;
    s_running = true;
    // fullwave ignored for digital mode — polarity toggle not supported
    ...
    mp_printf(&mp_plat_print, "pz_drive: fifo started (waveform=%u samples, gain=%d)\n",
              (unsigned)len, gain);
}
