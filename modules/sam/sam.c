...
void Init() {
    ...
    bufferpos = 0;
    // If buffer was pre-allocated externally (e.g. by MicroPython), keep it.
    if (buffer == NULL) buffer = malloc(22050 * 10);
    ...
    */
    ...
    phonemeindex[255] = 255; // to prevent buffer overflow // ML : changed from 32 to 255 to stop
                             // freezing with long inputs
}
...
// void Code41014()
void Insert(unsigned char position /*var57*/, unsigned char mem60, unsigned char mem59,
            unsigned char mem58) {
    int i;
...
void Parser2() {
                ...
                if (A == 0) {
                    ...
                    if (A != 0) {
                        ...
                        if (A != 0) {
                            ...
                            if (debug)
                                printf("RULE: Insert glottal stop between two stressed vowels with "
                                       "space between them\n");
                            // 31 = 'Q'
                            Insert(X, 31, mem59, 0);
                            pos++;
    ...
    pos41611:
        ...
        // RULE:
        //             K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR DIPHTONG NOT ENDING
        //             WITH IY>
        // Example: COW
        ...
        if (A == 72) // 'K'
        {
            ...
            if (Y == 255) phonemeindex[pos] = 75; // ML : prevents an index out of bounds problem
                ...
                if (debug)
                    if (A == 0)
                        printf("RULE: K <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> KX <VOWEL OR "
            ...
            // RULE:
            //             G <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> GX <VOWEL OR DIPHTONG NOT
            //             ENDING WITH IY>
            // Example: GO
            ...
            if (A == 60) // 'G'
            {
                ...
                if (debug)
                    printf("RULE: G <VOWEL OR DIPHTONG NOT ENDING WITH IY> -> GX <VOWEL OR "
                           "DIPHTONG NOT ENDING WITH IY>\n");
        ...
        // Replace with softer version
        if (debug)
            printf("RULE: S* %c%c -> S* %c%c\n", signInputTable1[Y], signInputTable2[Y],
                   signInputTable1[Y - 12], signInputTable2[Y - 12]);
...
void AdjustLengths() {
        ...
        do {
            ...
            if (index != 255) // inserted to prevent access overrun
                ...
                if (((flags2[index] & 32) == 0) || ((flags[index] & 4) != 0)) // nochmal �berpr�fen
                {
                    ...
                    A = (A >> 1) + A + 1;
                    if (debug)
                        printf("RULE: Lengthen <FRICATIVE> or <VOICED> between <VOWEL> and "
                               "<PUNCTUATION> by 1.5\n");
                    if (debug) printf("PRE\n");
                    if (debug)
    ...
    while (1) {
        ...
        if (A != 0) {
            ...
            // get flags
            if (index == 255) mem56 = 65; // use if end marker
            else mem56 = flags[index];
        ...
        if ((flags2[index] & 8) != 0) {
            ...
            else A = flags[index] & 2;    // check for stop consonant
...

            {
                ...
                if (debug)
                    printf("phoneme %d (%c%c) length %d\n", X - 1,
                           signInputTable1[phonemeindex[X - 1]],
                           signInputTable2[phonemeindex[X - 1]], phonemeLength[X - 1]);
        ...
        if ((flags[index] & 2) != 0) {
            ...
            if (debug)
                printf("RULE: <UNVOICED STOP CONSONANT> {optional silence} <STOP CONSONANT> - "
            ...
            if (debug)
                printf("phoneme %d (%c%c) length %d\n", debugX,
                       signInputTable1[phonemeindex[debugX]], signInputTable2[phonemeindex[debugX]],
