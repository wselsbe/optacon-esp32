...
void Render() {
    ...
    //    ------------------------------------------------
    // S
    //    241     0     6     0    73     0    99    61   Use S (weight 18) for transition instead
    //    of AH (weight 2) 241     0     6     0    73     0    99    61   <-- (OutBlendFrames-1) =
    //    (1-1) = 0 frames
    // AH
    ...
    //      0    15    22     9    44     1    87    53
    //      0    15    22     9    44     1    87    53   Use N (weight 8) for transition instead of
    //      AH (weight 2). 0    15    22     9    44     1    87    53   Since N is second phoneme,
    //      reverse the IN and OUT values. 0    11    17     8    47     1    98    56 * <--
    //      (InBlendFrames-1) = (2-1) = 1 frames
    // N
    ...
    //     0     5     6     5    54     0   121    61
    //     0     5     6     5    54     0   121    61
    //     0     5     6     5    54     0   121    61   Use N (weight 8) for transition instead of
    //     AH (weight 2) 0     5     6     5    54     0   121    61   <-- (OutBlendFrames-1) =
    //     (1-1) = 0 frames
    // AH
    //     0     8    11     6    51     0   110    59 * <-- InBlendFrames = 2
    //     0    11    16     8    48     0    99    56 *
    //     0    15    22     9    44     1    87    53   Use S (weight 18) for transition instead of
    //     AH (weight 2) 0    15    22     9    44     1    87    53   Since S is second phoneme,
    //     reverse the IN and OUT values. 0     9    18     5    51     1    90    55 * <--
    //     (InBlendFrames-1) = (3-1) = 2 0     4    14     3    58     1    93    57 *
    // S
        ...
        if ((X & 128) == 0)
            do // while No. 2
            {
                ...
                if (mem47 == 168) // pitch
                {
                    ...
                    A = Read(mem47,
                             mem37); // value at center of next phoneme - end interpolation value
                    // A = mem[address];
    ...
    if (debug) {
        PrintOutput(sampledConsonantFlag, frequency1, frequency2, frequency3, amplitude1,
                    amplitude2, amplitude3, pitches);
    }
