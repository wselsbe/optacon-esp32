...
void PrintPhonemes(unsigned char *phonemeindex, unsigned char *phonemeLength,
    ...
    while ((phonemeindex[i] != 255) && (i < 255)) {
        if (phonemeindex[i] < 81) {
            ...
            printf(" %3i      ??      %3i       %i\n", phonemeindex[i], phonemeLength[i],
                   stress[i]);
        }
        i++;
    }
...
void PrintOutput(unsigned char *flag, unsigned char *f1, unsigned char *f2, unsigned char *f3,
                 unsigned char *a1, unsigned char *a2, unsigned char *a3, unsigned char *p) {
    printf("===========================================\n");
    ...
    printf("------------------------------------------------\n");
    while (i < 255) {
        printf("%5i %5i %5i %5i %5i %5i %5i %5i\n", flag[i], a1[i], f1[i], a2[i], f2[i], a3[i],
               f3[i], p[i]);
        i++;
    }
    printf("===========================================\n");
