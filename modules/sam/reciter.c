...
int TextToPhonemes(unsigned char *input) {
    ...
    int r;
    // stack-allocate reciter context (258 bytes, no heap needed)
    SAMReciterContext ctx_storage;
    SAMReciterContext *ctx = &ctx_storage;
...
pos36700:
    ...
    while (1) {
        if (GetRuleByte(curRulePos, Y) != ctx->reciterInput[ctx->reciterIndex]) goto pos36700;
        if (++Y == rparenPos) break;
    ...
    while (1) {
        ...
        while (1) {
            ...
            if ((tab36376[ctx->reciterIndex] & 128) == 0) break;
            if (ctx->reciterInput[mem59 - 1] != mem57) goto pos36700;
            --mem59;
        }
        ...
        if (r == -1) {
            ...
            case '&':
                if (!Code37055(ctx, mem59 - 1, 16)) {
                    if (ctx->reciterInput[ctx->reciterIndex] != 'H') r = 1;
                    else {
                        ctx->workingChar = ctx->reciterInput[--ctx->reciterIndex];
                        if ((ctx->workingChar != 'C') && (ctx->workingChar != 'S')) r = 1;
                    }
                }
                break;
            ...
            case '@':
                if (!Code37055(ctx, mem59 - 1, 4)) {
                    ctx->workingChar = ctx->reciterInput[ctx->reciterIndex];
                    if (ctx->workingChar != 72) r = 1;
                    if ((ctx->workingChar != 84) && (ctx->workingChar != 67) &&
                        (ctx->workingChar != 83))
                        r = 1;
                }
                break;
            case '+':
                ...
                ctx->workingChar = ctx->reciterInput[--ctx->reciterIndex];
                if ((ctx->workingChar != 'E') && (ctx->workingChar != 'I') &&
                    (ctx->workingChar != 'Y'))
                    r = 1;
                break;
            case ':':
        ...
        if (ctx->reciterInput[ctx->reciterIndex] == 'E') {
            if ((tab36376[ctx->reciterInput[ctx->reciterIndex + 1]] & 128) != 0) {
                ...
                if (ctx->workingChar == 'L') {
                    if (ctx->reciterInput[++ctx->reciterIndex] != 'Y') goto pos36700;
                } else if ((ctx->workingChar != 'R') && (ctx->workingChar != 'S') &&
                           (ctx->workingChar != 'D') && !IsNextInput(ctx, "FUL"))
                    goto pos36700;
            }
        } else {
            if (!IsNextInput(ctx, "ING")) goto pos36700;
            ...
            while (1) {
                ...
                if (Y == mem64) {
                    ...
                    while (1) {
                        ...
                        if (ctx->workingChar != '=') input[++phonemeOutputPos] = ctx->workingChar;
            ...
            if (r == 0) {
                ...
                if (ctx->workingChar == '@') {
                    ...
                    while (Code37055(ctx, mem58 + 1, 32))
                        mem58 = ctx->reciterIndex;
                    r = -2;
                } else r = handle_ch(ctx, ctx->workingChar, mem58 + 1);
            }
...
unsigned int IsNextInput(SAMReciterContext *ctx, const char *str) {
    while (*str) {
        ...
        ctx->workingChar = ctx->reciterInput[ctx->reciterIndex++];
        if (ctx->workingChar != ch) return 0;
        ++str;
    }
    return 1;
}
...
int handle_ch(SAMReciterContext *ctx, unsigned char ch, unsigned char mem) {
    ...
    if (ch == ' ') {
        ...
        if ((tmp & 16) == 0) {
            if (ctx->reciterInput[ctx->reciterIndex] != 72) return 1;
            ++ctx->reciterIndex;
        }
    } else if (ch == '^') {
