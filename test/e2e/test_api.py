...
async def test_ota_config_get(test_app):
    ...
    url, deps = test_app
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{url}/api/ota/config") as r:
            assert r.status == 200
            data = await r.json()
...
async def test_ota_check(test_app):
    ...
    url, deps = test_app
    async with aiohttp.ClientSession() as s:
        async with s.post(f"{url}/api/ota/check") as r:
            assert r.status == 200
            data = await r.json()
...
async def test_ota_upload_firmware(test_app):
    ...
    url, deps = test_app
    async with aiohttp.ClientSession() as s:
        async with s.post(f"{url}/api/ota/upload", data=b"\x00" * 100) as r:
            assert r.status == 200
            data = await r.json()
            assert data["status"] == "ok"
...
async def test_ota_log(test_app):
    ...
    async with aiohttp.ClientSession() as s:
        async with s.get(f"{url}/api/ota/log") as r:
            assert r.status == 200
            data = await r.json()
...
async def test_ota_diagnostics(test_app):
    ...
    url, deps = test_app
    async with aiohttp.ClientSession() as s:
        async with s.post(f"{url}/api/ota/diagnostics") as r:
            assert r.status == 200
            data = await r.json()
            assert data["status"] == "ok"
...
async def test_ws_initial_status(test_app):
    ...
    url, deps = test_app
    async with aiohttp.ClientSession() as s:
        async with s.ws_connect(_ws_url(url)) as ws:
