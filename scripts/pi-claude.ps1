...
try {
    while ($true) {
        ssh -t -L 8224:127.0.0.1:8224 -R 8225:127.0.0.1:8225 optacon-pi 'n=0; while ! tmux has-session -t claude 2>/dev/null; do sleep 1; n=$((n+1)); if [ $n -ge 30 ]; then echo "Timeout waiting for claude session"; exit 1; fi; done; exec tmux attach -t claude'
        Write-Host ""
